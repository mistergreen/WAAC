...
void PWM4::setPins(int red, int green, int blue, int white, int channel0, int channel1, int channel2, int channel3 ) {
    ...
    for(uint8_t i = 0; i < NUM_CHANNELS; i++) {
        if(color[i].pin > UNSET) {
            ...
            ledcAttachChannel(color[i].pin,LEDC_BASE_FREQ,LEDC_TIMER_BIT, color[i].pwmChannel);
            //ledcAttach(color[i].pin, LEDC_BASE_FREQ, LEDC_TIMER_BIT);
        }
...
Serial.println("pwm de 1");
    Device::deserialize(doc);
Serial.println("pwm de 2");
    EventHandler::deserialize(doc);
Serial.println("pwm de 3");
    char event[4*20];
    // clean the buffer
    memset(event, 0, sizeof(event));

    if (doc.containsKey("eventColors")) {
        strcpy (event, doc["eventColors"]);
        setEventColors(event);
    }

Serial.println("pwm de 4");
    setPins(doc["pin0"].as<int>(),
            doc["pin1"].as<int>(),
            doc["pin2"].as<int>(),
            doc["pin3"].as<int>(),
            doc["cha0"].as<int>(),
