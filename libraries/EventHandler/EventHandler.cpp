...
void EventHandler::setUpTime()
{
           // Get the settings instance.
        settings = WWWsettings::getinstance();

        // Get the current time.
        now = settings->getTime();
        lastDay = now->day();
}
...
bool EventHandler::isEventInProgress()
{
    ...
    if ((-1 < inProgressEventId) && (sMAX_NUM_EVENTS > inProgressEventId))
    {
        ...
        time_t currentTime = makeTime(now->hour(), now->minute(), now->second(), now->day(), now->month(), now->year());
        // Get start time

        time_t eventTime = makeTime(hour[inProgressEventId], minute[inProgressEventId], second[inProgressEventId], lastDay, now->month(), now->year());
                   
        // Calculate duration
        long duration = convertToSeconds(hourDuration[inProgressEventId],
                                            minuteDuration[inProgressEventId],
                                            secondDuration[inProgressEventId]);
        ...
        if ( currentEventDow == '1' && currentTime - eventTime >= 0 && currentTime - eventTime <= duration ) {
            retVal = true;  
        }
        else {
    ...
    // Loop through events and check if there is one triggered. More than One event to a device!
    ...
    for (uint8_t i = 0; (i < timedIndexCounter) && (false == retVal); i++) {
        ...
        if(dow[i][::weekday()-1] == '1') {
            ...
            time_t eventTime = makeTime(hour[i], minute[i], second[i], lastDay, now->month(), now->year());
            
            // Calculate duration
            long duration = convertToSeconds(hourDuration[i],
                                                minuteDuration[i],
                                                secondDuration[i]);
   
            if ( currentTime - eventTime >= 0 && currentTime - eventTime <= duration ) {
                retVal = true;
                inProgressEventId = i;
            } else {
                lastDay = now->day();
    ...
    JsonObject& doc)
{
    ...
    setEvent(event);
    // coming in random int if null
    if (doc.containsKey("dependent"))
        setDependentDevice(doc["dependent"].as<int>());
    if (doc.containsKey("suspendTime"))
        setSuspendTime(doc["suspendTime"].as<bool>());
}
