...
void SensorWaac::deserialize(
    JsonObject& doc)
{
    if (doc.containsKey("formula"))
        strcpy(formula, doc["formula"]);

    if (doc.containsKey("conditionType"))
        strcpy(conditionType, doc["conditionType"]);
    
    if (doc.containsKey("lowerTarget"))
        lowerTarget = doc["lowerTarget"].as<float>();

    if (doc.containsKey("higherTarget"))
        higherTarget = doc["higherTarget"].as<float>();
