void testEmail(EthernetClient client) {
...
void testIp(EthernetClient client) {
...
void testNTP(EthernetClient client) {
      webParser.clearBuffer(param_value, queryMax);
      webParser.parseQuery(queryBuffer, "testntp", param_value);
      wwws.setNTPip(param_value);
...
void testDNS(EthernetClient client) {
   webParser.clearBuffer(param_value, queryMax);
...
void saveSetting(EthernetClient client) {
      webParser.clearBuffer(param_value, queryMax);
      webParser.parseQuery(queryBuffer, "email", param_value);
      wwws.setToEmail(param_value);
...
void getSetting(EthernetClient client) {
       client.println(F("HTTP/1.1 200 OK"));
      client.println(F("Content-Type: text/xml"));
      client.println();
